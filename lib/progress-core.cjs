...
// 랭커 보드 정렬: 레벨 내림차순 → 경험치 내림차순(null은 최하위) → 이름 오름차순.
// 경험치 소스는 user/getStudyUsers (mbrId 정확 매칭) — 미발견 멤버는 exp null로 남는다.
// members[]의 progress는 current.json 직렬화 형태(객체) 기준. 완료 과제 수를 부가 계산한다.
// 공개 파일 프라이버시: profImg/recntCntnDt는 싣지 않는다 (2026-07-25 프라이버시 점검).
function rankEntries(members) {
  const rows = (members || []).map((m) => {
    let done = 0;
    for (const v of Object.values(m.progress || {})) if (v && v.st === "C") done += 1;
    return {
      mbrId: m.mbrId,
      name: m.name,
      guild: m.guild || "미배정",
      level: m.level != null ? Number(m.level) : null,
      exp: m.exp != null && Number.isFinite(Number(m.exp)) ? Number(m.exp) : null,
      pnt: m.pnt != null && Number.isFinite(Number(m.pnt)) ? Number(m.pnt) : null,
      done,
    };
  });
  rows.sort((a, b) =>
    (b.level || 0) - (a.level || 0) ||
    (b.exp != null ? b.exp : -1) - (a.exp != null ? a.exp : -1) ||
